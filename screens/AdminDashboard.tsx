              ...
              <div className="flex gap-6 items-center flex-1">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${b.status === 'Confirmed' ? 'bg-green-500 text-white' :
                        b.status === 'Declined' || b.status === 'Cancelled' ? 'bg-red-500 text-white' :
                          b.status === 'Refunded' ? 'bg-slate-500 text-white' :
                            'bg-primary text-white'
                      }`}>
                      {b.status}
                      {(b.status === 'Confirmed' || b.status === 'Declined' || b.status === 'Cancelled') && (
              ...
              <div className="flex gap-3 w-full md:w-auto">
                ...
                {b.status === 'Confirmed' && (
                  <>
                    ...
                    <button
                      onClick={() => handleStatusUpdate(b.id, 'Cancelled', b.email, b.customerName)}
                      className="w-full md:w-auto px-8 py-3 rounded-xl border border-border-dark text-slate-500 font-black uppercase tracking-widest text-xs hover:border-orange-500 hover:text-orange-500 transition-all"
                    >
                      Cancel Booking
                    </button>
                  </>
                ...
                {(b.status === 'Refunded' || b.status === 'Cancelled') && (
                  <div className="flex items-center gap-2 text-slate-400 font-black uppercase italic text-xs tracking-widest bg-slate-100 dark:bg-background-dark px-6 py-3 rounded-xl">
                    <span className="material-symbols-outlined text-sm">{b.status === 'Refunded' ? 'history' : 'cancel'}</span>
                    {b.status}
                  </div>
                )}
              </div>
